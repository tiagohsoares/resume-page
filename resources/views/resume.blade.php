<x-layout :title=" $resume->basics->name . ' Resume' ">
      ...
      <div class="mt-1 flex flex-wrap">
        @foreach ($resume->basics->profiles as $profiles)
          <a href="{{$profiles->url}}" target="_blank"
            class="px-3 py-1.5 bg-white rounded-full shadow-sm text-sm hover:underline text-gray-700">{{$profiles->network}}</a>
        @endforeach
      </div>
    @endif
  ...
  @if ($allowDownload)
    <x-download-resume-button :allowDownload="$allowDownload" />
  @endif


  <section class="mt-10">
    <h2 class="text-2xl font-semibold border-b border-gray-200 pb-2 mb-4">Experiência</h2>
    <div class="space-y-6">
          ...
          @endif
            ...
            @if(!empty($work->startDate))
                <div class="text-gray-600 text-sm mt-1">
                  {{$work->startDate->format("M Y")}} -
                  {{$work->endDate ? $work->endDate->format("M Y") : "Atual" }}
                </div>
              </div>
            @endif

          <div class="mt-2">
            <a href="{{$work->url}}" target="_blank" class="mt-4 text-sm hover:underline text-blue-600">
              {{$work->url}}</a>
              ...
              <div class="flex justify-between items-center w-full mb-2">
                ...
                @if(!empty($skills->level))
                  <div class="text-sm mt-0">
                    <span
                      class="ml-7 inline-block items-center rounded-md bg-blue-100 px-2 py-1 font-medium text-blue-700 ring-1 ring-blue-700/10">{{$skills->level}}</span>
                  </div>
                @endif
              </div>
              ...
              @if(!empty($skills->keywords))
                <div class="mt-2 flex flex-wrap gap-2">
                  @foreach ($skills->keywords as $keywords)
                    <span
                      class="ml-1 inline-block items-center rounded-sm bg-gray-100 px-2 py-1 font-medium text-gray-700">{{$keywords}}</span>
                  @endforeach
              @endif
              </div>
            </div>
      ...
      <section>
        <h2 class="text-2xl font-semibold border-b border-gray-200 pb-2 mb-4">Projeto</h2>
        @foreach ($resume->projects as $projects)
          <div class="bg-white p-5 shadow-sm rounded-lg">
            <div class="flex flex-row-2 justify-between gap-6">
              ...
              @if(!empty($projects->startDate))
                <div class="text-sm text-gray-600">
                  {{$projects->startDate->format("M Y")}} -
                  {{$projects->endDate ? $projects->endDate->format("M Y") : "Atual" }}
                </div>
              @endif
            </div>
            ...
            @if (!empty($projects->url))
              <div>
                <a href="{{$projects->url}}" target="_blank" class="text-blue-500 hover:underline text-sm">
                  {{ $projects->url }} </a>
              </div>
            @endif
      ...
      <section>
        <h2 class="text-2xl font-semibold border-b border-gray-200 pb-2 mb-4">Educação</h2>
        @foreach($resume->education as $education)
          <div class="bg-white p-5 shadow-sm rounded-lg mb-3">
            <div>
              <h3 class="text-2xl font-semibold text-gray-800">{{$education->institution}}</h3>
            </div>
            @if(!empty($education->startDate))
              <div class="text-sm text-gray-600 mt-1">
                {{$education->startDate->format("M Y")}} -
                {{$education->endDate ? $education->endDate->format("M Y") : "Atual" }}
              </div>
            @endif
            <div>
              <h3>{{ $education->area  }}</h3>
            </div>
            @if(!empty($education->courses))
            <ul class="list-disc list-outside mt-2 space-y-0.5 ml-4">
              @foreach($education->courses as $courses)
                <div class="flex-wrap flex justify-beetween gap-4 text-gray-700">
                  <li class="pl-5">{{ $courses }}</li>
                </div>
              @endforeach
          </ul>
              @endif
